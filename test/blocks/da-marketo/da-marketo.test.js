...
import { setConfig, getConfig, MILO_EVENTS } from './mocks/utils.js';
...
import init, {
  ...
  FORM_ID_PARAM,
  NAMED_FORM_IDS,
} from '../../../mkto/blocks/da-marketo/da-marketo.js';
...
const config = {
  codeRoot: '/test/blocks/da-marketo/mocks',
...
describe('da-marketo decorateURL', () => {
  it('decorates absolute URL with local base URL', async () => {
    const baseURL = new URL('http://localhost:6586/marketo-block');
    const result = await decorateURL('https://main--da-marketo--adobecom.aem.page/marketo-block/thank-you', baseURL);
    expect(result).to.equal('http://localhost:6586/marketo-block/thank-you');
  });

  it('decorates relative URL with absolute base URL', async () => {
    const baseURL = new URL('https://main--da-marketo--adobecom.aem.page/marketo-block');
    const result = await decorateURL('/marketo-block/thank-you', baseURL);
    expect(result).to.equal('https://main--da-marketo--adobecom.aem.page/marketo-block/thank-you');
  });
...
describe('da-marketo ungated one page experience', () => {
  let url;
  let clock;
...
describe('da-marketo failure logging', () => {
  let clock;
  let el;
  ...
  describe('loadMarketo', () => {
    ...
    it('logs a forms2/mkto pipeline load failure and hides the block', async () => {
      buildBlock();
      window.mktoMockFail = true;
      await loadMarketo(el, formData);
...
describe('da-marketo formSuccess IMS', () => {
  let formEl;
  let marketoEl;
...
describe('da-marketo form id param', () => {
  let url;
