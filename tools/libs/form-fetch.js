...
export { buildGetFormBaseUrl } from './defaults.js';
...
export function getFlattenedFields(formData) {
...
function extractFromHtmltext(htmltext, state, parser, formId) {
 ...
 * @param {{ formId?: number|null }} [options]
 */
export function extractFromFormData(raw, options = {}) {
  const { formId = null } = options;
  ...
  for (let i = 0; i < fields.length; i += 1) {
    ...
    if (typeof ht === 'string' && ht.length) {
      extractFromHtmltext(ht, state, parser, formId);
    }
  }
