  ...
  function marketoFormSetup(lvl) {
    ...
    if (lvl == "stage1") {
      ...
      function checkTemplate() {
        ...
        if (!templateRule) {
          mkf_c.warn("Template no rule: " + templateName);
          mkf_c.groupEnd(groupLBL);
          return;
        }
    ...
    window.MktoForms_onSuccess = function () {
      ...
      ty_content = ty_content.trim().toLowerCase();
      ...
      if (ty_type == "adobe_connect") {
        if (typeof window.mcz_marketoForm_adobe_connect_event == "function") {
          ty_content = "adobe_connect";
          if (MktoFormsValidUrl(ty_content) == true) {
            logSuccess("Connect URL");
            try {
            ...
            logSuccess("Connect URL=X >> message.");
            ty_content = "Adobe Connect, URL is invalid.";
          }
        } else {
          mkf_c.log(
            "window.mcz_marketoForm_adobe_connect_event is not defined, setting to message"
          );
          ty_type = "message";
          ty_content = "";
        }
      ...
      if (ty_content != "" && ty_type != "adobe_connect") {
        if (MktoFormsValidUrl(ty_content) == true) {
          logSuccess("TY URL");
      ...
      if (ty_type == "redirect") {
        ...
        if (ty_type == "adobe_connect") {
          try {
            window.mcz_marketoForm_adobe_connect_event(ty_content);
          } catch (e) {
            mkf_c.log("window.mcz_marketoForm_adobe_connect_event error", e);
          }
        }
