...
if (typeof setRequired != "function") {
  console.log("Field Preferences - Loaded");
  const fieldMap = {};
  ...
  if (window?.mcz_marketoForm_pref?.value_setup?.fieldDependance) {
    for (var key in window?.mcz_marketoForm_pref?.value_setup?.fieldDependance) {
      if (window?.mcz_marketoForm_pref?.value_setup?.fieldDependance.hasOwnProperty(key)) {
        fieldDependance[key.toLowerCase()] =
          window?.mcz_marketoForm_pref?.value_setup?.fieldDependance[key];
      }
    }
  }
  //
  ...
  function setRequired(fieldName, direction) {
    if (fieldName == "" || fieldName == null || fieldName == undefined) {
      console.log("setRequired - fieldName is empty");
      return;
    }
    ...
    if (typeof setRequired_set[fieldName] !== "number") {
        ...
        if (row) {
          ...
          if (mktoInstruction) {
            ...
            if (fieldDependance[fieldName.toLowerCase()]) {
              let fieldDependanceName = fieldDependance[fieldName.toLowerCase()];
  ...
  function checkDataLayer() {
    ...
    if (!marketoConfigExists && fieldVisibilityCheckInterval) {
      ...
      console.log("Marketo Config Interface no longer exists. Stopping field visibility check.");
      return;
    }
