...
if (typeof cleaning_validation != "function" && typeof form_dynamics !== "undefined") {
  function cleaning_validation() {
    ...
    function normalizeMktoStyles() {
      ...
      let mktoFields = mktoForm.querySelectorAll(".mktoField[name]:not(.mktofield-anchor)");
          ...
          if (
            ...
            while (mktoFieldParent != null) {
              if (
                ...
                mktoField.classList.add("mktofield-anchor");
                mktoFieldParent.setAttribute("data-mktofield", mktoField.getAttribute("name"));
                let fldStepPref = window?.mcz_marketoForm_pref?.form?.fldStepPref;
                if (fldStepPref) {
                  for (let step in fldStepPref) {
                    if (
                      Array.isArray(fldStepPref[step]) &&
                      fldStepPref[step].includes(mktoField.getAttribute("name"))
                    ) {
                      let totalSteps = mktoForm.querySelectorAll(
                        `.mktoFormRow[data-mktofield_step="${step}"]`
                      ).length;
                      if (totalSteps === 0) {
                        mktoFieldParent.classList.add("mktofield-first-of-step");
                      }
                      mktoFieldParent.dataset.mktofield_step = step;
                      mktoField.dataset.mktofield_step = step;
                    }
                  }
                  let firstofstep = mktoForm.querySelectorAll(".mktofield-first-of-step").length;
                  mktoForm.dataset.mktofield_step_total = firstofstep;
                }
                break;
              }
      ...
      try {
        const checkSetup = mktoForm.querySelectorAll(".mkto-step-setup").length;
        const visibleStepRows = mktoForm.querySelectorAll(
          ".mktoFormRowTop:not(.mktoHidden)[data-mktofield_step]"
        );
        let lowestStep = 99;
        let lowestStepCurrent = 99;
        if (visibleStepRows.length > 0) {
          lowestStepCurrent = mktoForm.dataset?.mktofield_step_lowest || 99;
          lowestStep = Math.min(
            ...Array.from(visibleStepRows).map((row) => parseInt(row.dataset.mktofield_step, 10))
          );
        }
        if (
          checkSetup == 0 ||
          (lowestStep < 99 && lowestStep !== parseInt(lowestStepCurrent, 10))
        ) {
          const backBtn = mktoForm.querySelector(".mkto-step-back-btn");
          const submitBtn = mktoForm.querySelector("#mktoButton_new");
          const maxSteps = mktoForm?.dataset?.mktofield_step_total || 0;

          const stepsDisplay = mktoForm.querySelectorAll(".mkto-step00of99");
          if (stepsDisplay.length > 0) {
            const templateText = stepsDisplay[0]?.getAttribute("data-mkto-btn-step00of99") || "";

            mktoForm.dataset.mktofield_step_lowest = lowestStep;

            mkf_c.log("Lowest Step: " + lowestStep + " Max Steps: " + maxSteps);

            if (maxSteps > 0 && templateText.length > 0) {
              let replaceText = templateText;

              if (lowestStep !== 99) {
                replaceText = replaceText.replace("00", lowestStep.toString());
                replaceText = replaceText.replace("99", maxSteps.toString());
              } else {
                replaceText = "";
                mkf_c.log("Steps are not set correctly, removing step display");
              }
              stepsDisplay.forEach((stepDisplay) => {
                stepDisplay.innerText = replaceText;
              });
            }
          }
          if (backBtn) {
            if (lowestStep > 1) {
              backBtn.classList.remove("mktoHidden");
            } else {
              backBtn.classList.add("mktoHidden");
            }
          }
          if (submitBtn) {
            const txtSubmit = submitBtn?.getAttribute("data-mkto-btn-submit") || "Submit";
            const txtNext = submitBtn?.getAttribute("data-mkto-btn-next") || "Next";
            if (lowestStep === maxSteps) {
              submitBtn.innerText = txtSubmit;
            } else {
              submitBtn.innerText = txtNext;
            }
            mkf_c.log("STEP Management: Submit Button Text: " + submitBtn.innerText);
            submitBtn.classList.add("mkto-step-setup");
          }
        }
      } catch (e) {
        mkf_c.log("STEP Management Error: ", e);
      }
