...
if (typeof mktoPerformanceObserver == "undefined") {
  ...
  window.checkCookie = function (incMunchkin) {
    if (incMunchkin == undefined) {
      incMunchkin = true;
    }
    if (incMunchkin) {
      return (
  ...
  async function confirmAdobeProfile(form) {
    try {
      if (
        window.location.href.indexOf("preview=1") > -1 &&
        window.location.href.indexOf("formId=") > -1
      ) {
        if (form === null || form === undefined) {
          mkf_c.warn("Form is null or undefined for Adobe Profile");
          return;
        }
        if (typeof window?.adobeIMS?.profileService?.getProfile === "function") {
          const profile = await window.adobeIMS.profileService.getProfile();
          if (profile) {
            ...
            let email = ap?.email;
            if (email) {
              if (email.indexOf("@adobe.") > -1) {
                email = email.replace("@adobe.", "@adobetest.");
                ap.mkto_FormsCompany = "Adobe";
              }
            }
            ap.Email = email;
            ap.FirstName = ap?.first_name;
            ap.LastName = ap?.last_name;
            ap.Phone = ap?.phoneNumber;
            ap.Country = ap?.countryCode;
            if (Array.isArray(ap?.preferred_languages)) {
              ap.prefLanguage = ap?.preferred_languages[0];
            } else {
              ap.prefLanguage = ap?.preferred_languages;
            }
            if (ap.prefLanguage) {
              ap.prefLanguage = ap.prefLanguage.trim().toLowerCase().replace("-", "_");
            }
            window.mcz_marketoForm_pref.profile.adobeProfile = ap;
            if (typeof form.setValues === "function") {
              form.setValues(mcz_marketoForm_pref.profile.adobeProfile);
            }
          }
        }
      }
    } catch (error) {
      mkf_c.warn("confirmAdobeProfile ", error);
    }
  }
  ...
  window.updateFormField = function (form, fieldName, value, critical = false) {
    if (form === null || form === undefined) {
  ...
  function marketoFormSetup(lvl) {
    ...
    if (lvl == "stage1") {
      ...
      let templateLog = "";
      function appendTemplateLog(log) {
      ...
      function checkTemplate() {
        ...
        appendTemplateLog("Subtypes Verbs");
        appendTemplateLog(JSON.stringify(mczPrefs.form.subtypeRules, null, 2));
        appendTemplateLog("\nSubtypes Enforced:");
        appendTemplateLog(JSON.stringify(mczPrefs.form.subtypeTemplate, null, 2));
        ...
        if (mczPrefs.form.subtypeTemplate) {
          if (mczPrefs.form.subtypeTemplate.hasOwnProperty(mczPrefs.form.template)) {
            ...
            if (originalSubtype != mczPrefs.form.subtype) {
              appendTemplateLog(
                "Subtype Changed: '" + originalSubtype + "' > '" + mczPrefs.form.subtype + "'"
              );
            }
          } else {
            appendTemplateLog("No template rule: " + mczPrefs.form.template);
            appendTemplateLog("Using subtype: " + mczPrefs.form.subtype);
          }
        } else {
          appendTemplateLog("subtype in template undefined.");
        }
        ...
        //flex is in the name and we don't have a query param of template
        if (templateName.indexOf("flex") > -1 && !mktoFrmParams.get("template")) {
          ...
          if (templateName.indexOf("flex") > -1 && mktoFrmParams.get("template") > -1) {
            appendTemplateLog("Testing Flex Form Detected - Rules Enforced");
          }
        ...
        appendTemplateLog(JSON.stringify(rule, null, 2));
      ...
      function applyRuleToFields(ruleFields, fields, fieldName) {
        ...
        appendTemplateLog(matches_Y + matches_N);
      }
    ...
    function mkto_buildForm() {
      ...
      MktoForms2.getForm(formId).onValidate(function (valid) {
        ...
        if (formData.hasOwnProperty("Email")) {
          ...
          let emailSplit = formData.Email.split("@");
          if (emailSplit.length == 2) {
            if (emailSplit[1].indexOf(".") == -1) {
              emailValid = false;
            }
          }
          let emailField = document.querySelector("#mktoForm_" + formId + ' [name="Email"]');
          if (emailField) {
            if (emailValid) {
              emailField.classList.remove("mktoInvalid");
              emailField.classList.add("mktoValid");
            } else {
        ...
        function mktoFrmsGetValueByName(name) {
          ...
          const getValue = () => {
            if (!consentCheck) return getParam(name);
            return (
              getParam(name) ||
        ...
        try {
          ...
          function addUniqueParam(paramName, paramValue) {
            ...
            if (paramValue !== "" && paramValue !== null && paramValue !== undefined) {
              if (mktoFrmPvtURL.searchParams && mktoFrmPvtURL.searchParams.has(paramName)) {
                mktoFrmPvtURL.searchParams.delete(paramName);
              }
              mktoFrmPvtURL.searchParams.set(paramName, paramValue);
            }
          }
          ...
          if (mktoTreatmentId !== "" && mktoTreatmentId !== null && mktoTreatmentId !== undefined) {
            ...
            updateFormField(form, "mktoTreatmentId", mktoTreatmentId);
          }
          ...
          if (gclid) {
            ...
            updateFormField(form, "mktoGoogleClickId", gclid);
          }
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            if (value != "" && value != null && value != undefined) {
              ...
              if (valid) {
                updateFormField(form, attrMapping[key].field, value, attrMapping[key].critical);
                if (attrMapping[key].status) {
                  updateFormField(
                    form,
                    attrMapping[key].status,
                    "Responded",
                    attrMapping[key].critical
                  );
                }
              } else {
                updateFormField(form, attrMapping[key].field, "", attrMapping[key].critical);
              }
              addUniqueParam(attrMapping[key].queryParam, value);
            }
          }
        ...
        updateFormField(form, "mktoformType", mcz_marketoForm_pref?.form?.type, true);
        updateFormField(form, "mktoformSubtype", mktoformSubtype, true);
        updateFormField(form, "languagePref", mcz_marketoForm_pref?.profile?.segLangCode, true);
        updateFormField(form, "mktoConsentURL", mktoFrmPvtURL, true);
        updateFormField(
          form,
          "mktoFormsPrimaryProductInterest",
          mcz_marketoForm_pref?.program?.poi,
          true
        );
        ...
        form.addHiddenFields({ mktoFormsTemplate: mktoFormsTemplate });
        ...
        form.addHiddenFields({ MktoSessionSubmissionID: unique_id });

        let mktoSessionSubscriptionID = window?.mcz_marketoForm_pref?.program?.subscription?.id;

        if (
          mktoSessionSubscriptionID !== undefined &&
          mktoSessionSubscriptionID !== null &&
          mktoSessionSubscriptionID !== ""
        ) {
          mktoSessionSubscriptionID = mktoSessionSubscriptionID.trim();
          form.addHiddenFields({ mktoSessionSubscriptionID: mktoSessionSubscriptionID });
        }
        ...
        if (activeCookie == true) {
          try {
            ...
            //  mkf_c.log("Error getting GUID: " + err);
          }
        ...
        let poi_warn = formData["mktoFormsPrimaryProductInterest"];
        if (poi_warn == "") {
        ...
        let mktoProductionCampaignId_warn = formData["mktoProductionCampaignId"];
    ...
    window.MktoForms_onSuccess = function () {
      ...
      if (ty_content != "" && ty_content != "null" && ty_content != "undefined") {
        ...
        logSuccess("Form Success Content not found, looking for lang specific version");

        if (translateFormElems?.thankyou) {
          let language = mcz_marketoForm_pref?.profile?.prefLanguage;
          let tyFallback = "Thank you for your submission.";
          let translatedTY =
          ...
          if (language) {
            ...
            if (translatedTY === null) {
              mkf_c.log(`No translation found for language: ${language}`);
            } else {
              mkf_c.log(`Translation found for language: ${language}`);
            }
          }
          ...
          if (translatedTY === null) {
            ...
            mkf_c.log(
              `Check General_Translations, No translated 'thankyou' text found for language: ${language}`
            );
          }
