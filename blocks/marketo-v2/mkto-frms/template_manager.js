...
if (typeof templateManager != "function" && typeof form_dynamics !== "undefined") {
  function templateManager() {
    mktoCss_LP();

    var template_2_fields = {
    ...
    function checkTemplateOptions(templateRules) {
      ...
      if (errors.length > 0) {
        mkf_c.log("Bad News, Errors found in templateRules.js file, please review and fix.");
        mkf_c.log(errors);
        return false;
      } else {
        mkf_c.log("Good News. No errors found in templateRules.js file.");
        return true;
      }
    }
    ...
    function checkOptionExists(fieldId, valOption, template) {
      ...
      let fieldKey = "";
      Object.keys(template_2_fields).find(function (key) {
        if (key === fieldId) {
          fieldKey = template_2_fields[key];
        }
      });
      if (!fieldKey) {
        mkf_c.log("Field '" + fieldId + "' is missing in the template_2_fields object.");
        return false;
      }
      ...
      const optionFound = options.some(function (opt) {
    ...
    function checkNestedFields(nestedFields, templateName, parentFieldName) {
      Object.keys(nestedFields).forEach(function (fieldName) {
        if (nestedFields[fieldName][0].split(",").length > 1) {
          var options = nestedFields[fieldName][0].split(",");
    ...
    function setTemplate(templateValue) {
      ...
      if (!templateValue && templateValue !== "") {
        mkf_c.log("Template is value is empty.");
        return;
      } else {
        ...
        if (!myRules) {
          mkf_c.log("Template " + templateValue + " does not exist in templateRules.js file.");
          return;
        } else {
          buildTemplates();
          ...
          if (myRules.field_visibility) {
            templateLog += "## \n## Field Visibility:\n";
            const field_visibility = myRules.field_visibility;
            Object.keys(field_visibility).forEach((key) => {
              ...
              if (fieldId) {
                ...
                // mkf_c.log(document.getElementById("field_visibility." + key));
              }
            });
          }
          if (myRules.field_filters) {
            templateLog += "## \n## Field Filters:\n";
            const field_filters = myRules.field_filters;
    ...
    if (marketoConfigExists) {
      mkf_c.log("Template Manager - Loaded");
      let templateSrc = document.getElementById("form.template");
      let templateOptions = templateSrc?.querySelectorAll("option");
      if (templateRules.length > 0 && templateOptions.length > 0) {
        ...
        if (
          ...
          let checkRules = checkTemplateOptions(templateRules);
          if (checkRules) {
          ...
          mkf_c.log(errorMessage);
          buildTemplates();
        } else {
          mkf_c.log("templateRules does not exist, please review and fix.");
          buildTemplates();
        }
      }
    }
  ...
  function mktoCss_LP() {
    ...
    if (params.has("isPreview") && window.location.href?.indexOf("/landingPage/formContent") > -1) {
      function addSelectBox() {
        ...
        var activeTemps = templateRules.map(function (template) {
        ...
        activeTemps.forEach(function (temp) {
          var option = document.createElement("option");
          option.value = temp;
        ...
        if (params.has("template")) {
          let currTemp = params.get("template");
          if (currTemp) {
      ...
      function addLangSelectBox() {
        ...
        var activeLangs = Object.keys(translateFormElems.lang);
        ...
        activeLangs.forEach(function (temp) {
          var option = document.createElement("option");
          option.value = temp;
          option.textContent = temp;
          selectBox.appendChild(option);
